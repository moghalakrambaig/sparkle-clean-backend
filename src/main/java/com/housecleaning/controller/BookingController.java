...
public class BookingController {
    ...
    public ResponseEntity<ApiResponse<Booking>> updateBookingStatus(
        ...
        return updated.map(booking ->
                        ResponseEntity.ok(new ApiResponse<>(true, booking, "Booking status updated")))
                .orElseGet(() ->
                        ResponseEntity.status(404).body(new ApiResponse<>(false, null, "Booking not found")));
    }
    ...
    public ResponseEntity<ApiResponse<Booking>> getBookingByNumber(@PathVariable String bookingNumber) {
        ...
        return booking.map(b ->
                        ResponseEntity.ok(new ApiResponse<>(true, b, "Booking found")))
                .orElseGet(() ->
                        ResponseEntity.status(404).body(new ApiResponse<>(false, null, "Booking not found")));
    }
}
